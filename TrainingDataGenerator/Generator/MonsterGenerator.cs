...
{
    public static List<Monster> GetRandomMonsters(CRRatios ratio, List<byte> levels, List<MonsterDifficulty> monsters)
    {
        ...
        Logger.Instance.Verbose($"EXP Thresholds calculated for party levels {string.Join(", ", levels)}: Cakewalk {expThresholds.Cakewalk}, Easy {expThresholds.Easy}, Medium {expThresholds.Medium}, Hard {expThresholds.Hard}, Deadly {expThresholds.Deadly}, Impossible {expThresholds.Impossible}");
        ...
        Logger.Instance.Verbose($"Floor Target XP before multiplier: {floorTargetExpBeforeMultiplier}");
        Logger.Instance.Verbose($"Target XP before multiplier: {targetExpBeforeMultiplier}");

        do
        {
            monstersFiltered = (targetExpBeforeMultiplier == -1) ? monsters : monsters.Where(m => m.Xp <= targetExpBeforeMultiplier).ToList();

            Logger.Instance.Verbose($"Found {monstersFiltered.Count} monsters matching criteria.");

            if (monstersFiltered.Count == 0)
            {
                if (randomNumMonsters == 15)
                {
                    Logger.Instance.Warning("Maximum number of monsters reached but no suitable monsters found. Resetting selection.");
                    randomNumMonsters = 1;
                }
                ...
                Logger.Instance.Verbose($"New Floor Target XP before multiplier: {floorTargetExpBeforeMultiplier}");
                Logger.Instance.Verbose($"New Target XP before multiplier: {targetExpBeforeMultiplier}");
            }
        }
        ...
        Logger.Instance.Verbose($"Selected monsters XP sum: {randomMonsters.Sum(m => m.Xp)}");
        Logger.Instance.Verbose($"Selected Monsters XP after multiplier: {randomMonsters.Sum(m => m.Xp) * numMultiplier}");
